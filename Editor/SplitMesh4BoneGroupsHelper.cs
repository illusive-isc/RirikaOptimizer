...
namespace jp.illusive_isc
{
    public static class SplitMesh4BoneGroupsHelper
    {
        ...
        {
            var nui = AssetDatabase.LoadAssetAtPath<GameObject>(
                "Assets/RirikaOptimizer/GeneratedMeshes/ぬいぐるみ.prefab"
            );
            GameObject container;
            if (nui != null)
            {
                container = PrefabUtility.InstantiatePrefab(nui) as GameObject;
                if (container != null)
                {
                    container.name = nui.name;
                }
            }
            else
            {
                ...
                SplitMesh4BoneGroups splitter =
                    ScriptableObject.CreateInstance<SplitMesh4BoneGroups>();
                ...
                container = splitter.SplitAllGroups(
                    "ぬいぐるみ",
                    smr,
                    groups,
                    null,
