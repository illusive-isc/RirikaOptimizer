...
{
    [AddComponentMenu("")]
    internal class IllRirikaParamFaceGesture : IllRirikaParam
    {
        ...
        public IllRirikaParamFaceGesture Initialize(
            VRCAvatarDescriptor descriptor,
            AnimatorController animator,
            bool FaceGestureFlg,
            bool FaceLockFlg,
        ...
        public IllRirikaParamFaceGesture DeleteFx()
        {
            if (FaceGestureFlg || FaceLockFlg)
                ...
                )
                {
                            ...
                            if (state.state.name == "Fist")
                            {
                                ...
                                var newMotion = AssetDatabase.LoadAssetAtPath<Motion>(
                                    AssetDatabase.GUIDToAssetPath(
                                        "c246cbd1068b6384b8c4b752022f9c2e"
                                    )
                                );
                    ...
                    if (layer.name is "RightHand")
                    {
                        ...
                        foreach (var state in states)
                        {
                            if (state.state.name == "Fist")
                            {
                                ...
                                var newMotion = AssetDatabase.LoadAssetAtPath<Motion>(
                                    AssetDatabase.GUIDToAssetPath(
                                        "f51cf46f18de7fd4dabf3a70544a4963"
                                    )
                                );
                                state.state.motion = newMotion;
                            }
                    ...
                    foreach (var state in layer.stateMachine.states)
                        if (state.state.name is "Idle 0" or "Idle")
                            foreach (var t in state.state.transitions)
                                t.conditions = t
                                    .conditions.Where(c => c.parameter != "FaceLock")
                                    .ToArray();
        ...
        public IllRirikaParamFaceGesture DeleteParam()
        {
            if (FaceGestureFlg || FaceValFlg)
        ...
        public IllRirikaParamFaceGesture DeleteVRCExpressions(
